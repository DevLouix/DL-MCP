...
export async function handleReadFile(relPath: string, maxFileSize: number): Promise<ToolResult> {
  try {
...
export async function handleWriteFile(relPath: string, content: string): Promise<ToolResult> {
  try {
    const target = await safeResolve(relPath);
...
export async function handleEditFile(relPath: string, search: string, replace: string): Promise<ToolResult> {
  try {
    const target = await safeResolve(relPath);
...
export async function handleCopyFile(source: string, destination: string): Promise<ToolResult> {
  try {
...
export async function handleMoveFile(source: string, destination: string): Promise<ToolResult> {
  try {
    const resolvedSource = await safeResolve(source);
    const resolvedDest = await safeResolve(destination);
...
export async function handleDeletePath(relPath: string): Promise<ToolResult> {
  try {
    const target = await safeResolve(relPath);
    const stats = await fs.stat(target);
