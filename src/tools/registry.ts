...
export function registerAllTools(server: McpServer, logger: Logger): void {
  server.registerTool(
    "list_directory",
    {
  ...
  server.registerTool(
    ...
    withAuditLog<{ path: string }>(
      "read_file",
      ({ path: relPath }) => handleReadFile(relPath, config.maxFileSize),
      logger,
    ),
  );
  ...
  server.registerTool(
    ...
    withAuditLog<{ path: string }>(
      "get_file_info",
      ({ path: relPath }) => handleGetFileInfo(relPath),
      logger,
    ),
  );
  ...
  server.registerTool(
    ...
    withAuditLog<{ path: string; content: string }>(
      "write_file",
      ({ path: relPath, content }) => handleWriteFile(relPath, content),
      logger,
    ),
  );
  ...
  server.registerTool(
    ...
    withAuditLog<{ path: string; search: string; replace: string }>(
      "edit_file",
      ({ path: relPath, search, replace }) => handleEditFile(relPath, search, replace),
      logger,
    ),
  );
  ...
  server.registerTool(
    ...
    withAuditLog<{ source: string; destination: string }>(
      "copy_file",
      ({ source, destination }) => handleCopyFile(source, destination),
      logger,
    ),
  );
  ...
  server.registerTool(
    ...
    withAuditLog<{ source: string; destination: string }>(
      "move_file",
      ({ source, destination }) => handleMoveFile(source, destination),
      logger,
    ),
  );
  ...
  server.registerTool(
    ...
    withAuditLog<{ path: string }>(
      "delete_path",
      ({ path: relPath }) => handleDeletePath(relPath),
      logger,
    ),
  );
  ...
  server.registerTool(
    ...
    withAuditLog<{ pattern: string; path: string; caseSensitive: boolean }>(
      "search_grep",
      ({ pattern, path: scopePath, caseSensitive }) =>
        handleSearchGrep(pattern, scopePath, caseSensitive, config.ignoredPaths, config.workspaceRoot, config.maxFileSize),
      logger,
    ),
  );
  ...
  server.registerTool(
    ...
    withAuditLog<{ path: string }>(
      "create_directory",
      ({ path: relPath }) => handleCreateDirectory(relPath),
      logger,
    ),
  );
  ...
  server.registerTool(
    ...
    withAuditLog<{ url: string; method: string; headers?: Record<string, string>; body?: string }>(
      "make_http_request",
      ({ url, method, headers, body }) => handleHttpRequest(url, method, headers, body, config.maxHttpResponseSize),
      logger,
    ),
  );
}
