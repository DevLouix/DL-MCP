...
async function safeReadFile(filePath: string): Promise<string | null> {
  try {
    ...
    if (stat.size > config.maxFileSize) return `[File too large: ${(stat.size / 1024 / 1024).toFixed(1)}MB]`;
...
export async function handleAnalyzeProject(args: { path?: string }): Promise<GetPromptResult> {
  ...
  try {
    ...
    for (const entry of dir) {
      if (config.ignoredPaths.has(entry.name)) continue;
  ...
  if (pkgJson) {
    try {
      ...
      if (deps.express || deps.hono || deps.fastify) techStack.push(deps.express ? "Express" : deps.hono ? "Hono" : "Fastify");
...
export async function handleExplainFile(args: { path: string }): Promise<GetPromptResult> {
...
function extractStructure(content: string, _ext: string): string {
  ...
  for (const line of lines) {
    ...
    if (/^export\s+/.test(trimmed)) exports.push(trimmed);
    if (/^class\s+\w+/.test(trimmed)) classes.push(trimmed);
  ...
  if (classes.length > 0) parts.push(`**Classes:** ${classes.length}`);
  if (functions.length > 0) parts.push(`**Functions/Consts:** ${functions.length}`);
  return parts.length > 0 ? parts.join(" | ") : "No significant structural elements detected.";
}

export async function handleReviewChanges(_args: { path?: string }): Promise<GetPromptResult> {
...
export async function handleGenerateReadme(args: { path?: string }): Promise<GetPromptResult> {
  const targetPath = args.path || "";
  const resolved = await safeResolve(targetPath);
  const stat = await fs.stat(resolved);
  ...
  return text([
    ...
    isFile
      ...
      : `This is the root of \`${name}\`, a project containing ${entries.length} items${
          entries.length > 0 ? ` (${entries.slice(0, 15).join(", ")}${entries.length > 15 ? ", ..." : ""})` : ""
        }.`,
    ``,
    pkg ? [
      `## Installation`,
